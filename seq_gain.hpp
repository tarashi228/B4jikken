...
inline void seq_gain_test(const autd::ControllerPtr& autd) {
  ...
  int a, b, c, d, e;
  a = 50; b = 50; c = 20; d = 50; e = 5;
  constexpr auto point_num = 175;  // a+b+c+d+e
  const double rate = 2;
  ...
  for (auto i = 0; i < point_num; i++) {
    ...
    double z;
    if (static_cast<double>(i % point_num) < a) {
        // ‹ß‚Ã‚­
        radius = 3.0;
        z = static_cast<double>(i % point_num)* static_cast<double>(d) / static_cast<double>(a);
        amps = { val1, val1, val1, val1 };
    }
    else if (a <= static_cast<double>(i % point_num) && static_cast<double>(i % point_num) < static_cast<double>(a + b)) {
        // —¯‚Ü‚é
        radius = static_cast<double>(3) - static_cast<double>(2) * (static_cast<double>(i % point_num) - a) / static_cast<double>(b);
        z = static_cast<double>(d);
        amps = { val1, val1, val1, val1 };
    }
    else if (static_cast<double>(a + b) <= static_cast<double>(i % point_num) && static_cast<double>(i % point_num) < static_cast<double>(a + b + c)) {
        radius = 1.0;
        z = static_cast<double>(d);
        amps = { val1, val1, val1, val1 };
    }
    else if (static_cast<double>(a + b + c) <= static_cast<double>(i % point_num) && static_cast<double>(i % point_num) < static_cast<double>(a + b + c + e)) {
        radius = 1.0;
        z = static_cast<double>(d);
        amps = { val2, val2, val2, val2 };
    }
    else {
        // —£‚ê‚é
        radius = 1.0;
        z = (static_cast<double>(a + b + c + d + e) - static_cast<double>(i % point_num));
        amps = { val1, val1, val1, val1 };
    }
